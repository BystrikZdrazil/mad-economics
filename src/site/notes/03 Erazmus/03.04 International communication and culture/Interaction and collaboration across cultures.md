---
{"dg-publish":true,"permalink":"/03-erazmus/03-04-international-communication-and-culture/interaction-and-collaboration-across-cultures/","tags":["erasmus","eraICC"],"dg-note-properties":{"Date":"2026-05-18","Class":"International communication and culture","tags":["erasmus","eraICC"]}}
---

# Promotion mix
- Advertising
- Public relations
	- Maintaining an image
- Direct marketing
	- [[03 Erazmus/03.01 Digital sales optimisation/E-mail marketing\|E-mail marketing]]
- Sales promotion
	- Offers, sales, "*buy 2 get 1 free*"
- Personal selling

# Goods to service dominant logic
- Value creation moves from production to co-creation (*collaboration with customer*)
	- Making something to assisting customer in **their own value creation** process
	- Produced value to **co-created value**
	- Customers as isolated entities to **networks**
	- Customers as targets to resources
	- Efficiency as primary to efficiency through effectiveness

# Relationship marketing
- **Focus**
	- Customer retention
	- Customer commitment
	- Share of customer's business VS market share
- **Unique aspects**
	- One-on-one relationship
	- Interactive process, blurred identity, location, time
	- Value-added activity

# The [[03 Erazmus/03.01 Digital sales optimisation/CRM, automation and revenue model\|CRM, automation and revenue model]] concept
- From mass marketing to **one-on-one marketing**
- **SMCs**: Segment marketing, adapting to the needs of the segment (*long-term investment*) -> mass customization (*allow small adaptations of products and services*)
- **CRM**: Build and sustain long-term relationships with customers
- Based on relationship concept instead of marketing concept

# Elements of CRM
- Direct marketing
- Relationship marketing
- Database marketing
- One-to-one marketing

>[!important]+
>CRM is a **combination** of other **marketing concepts**. **IT** is a crucial part of CRM.

# Marketing database
Comprises of the **market data**, **relationship data**, and finally **company data** to paint a clear picture.

## Market data
**Consumers**
- NAP data
- Geographic data
- Socio-demographic data
- Lifestyle data

**B2B**
- Sector
- Company amplitude
- Decision-makers, influencers

## Relationship data
**Action data**
- Promotions
- Direct mail actions
- Fulfilment

**Reaction data**
- Response to direct mail
- Complaints
- Purchases

## Company data
- Departments
- Products
- Sales team
- Sales techniques

# The customer life cycle
- Value for company and customer loyalty increases over time

## 3 stages of customer life cycle
1. **Acquisition**
	- Recruiting
	- Forming relationships
2. **Retention**
	- Maintain relationships
3. **Extension**
	- Increase involvement
	- Buy bigger assortment

**Find ideal customers** -> Pareto's law (*80/20*) -> those who pay on time, share information, are loyal

# Culture and public relations
To create and enhance a favourable corporate image with various publics.

**Crisis**: To maintain goodwill by responding to criticism, explaining actions and countering messages that may damage the image.

Cultural variance in:
- Making contacts
- Managing relationships
- Disclosing information
- Developing arguments
- Dealing with nationalistic feelings

# PR objectives and tasks
![ICC_PR_objectives_and_tasks.png](/img/user/06%20Images/ICC_PR_objectives_and_tasks.png)

>[!example]+
>**Findus** - Horse meat scandal
>**Jensens bøfhus** - disputes of names
>**Føtex** - winter bun (*instead of xmas bun*)
>**SAS** - advertisement "*what is truly Scandinavian?*"

# Corruption
Corruption in business involves **abusing** entrusted **power** for **private gain**, often through **bribery**, **fraud**, or **extortion**.

Corruption damage reputation and causing severe **financial** and **legal risks**.

**Common forms** include **bribery** in public procurement, **embezzlement**, and **collusion with officials**. It **undermines competition** inflate costs, and reduces productivity.

Each **year**, in both **developed** and **developing** countries, over **1 trillion USD** is paid in **bribes**.