---
{"dg-publish":true,"permalink":"/03-erazmus/03-11-supply-chain-management/gartner/","tags":["erasmus","eraSCM"]}
---

Gartner is an organisation for business leaders that covers SCM

# Top 25 methodology
- To be considered for Gartner’s Top 25
	- Companies that **appear** on the most **recent Fortune Global 500** or **Forbes 2000** lists
	- Companies that generate **at least 50%** of their **revenue** from **physical products**
	- A **minimum** annual **revenue** of **USD 15 billion**
	- A **minimum ESG score** requirement (*S&P Global ESG score*) in order to be considered

| **Category**        | **Criteria**                     | **Weighting** |
| ------------------- | -------------------------------- | ------------- |
| *Business metrics*  | Return on physical assets (ROPA) | 5%            |
| *Business metrics*  | Changes in ROPA                  | 10%           |
| *Business metrics*  | Revenue growth                   | 10%           |
| *Business metrics*  | Inventory turns                  | 5%            |
| *Business metrics*  | ESG                              | 20%           |
| *Community opinion* | Peer vote                        | 25%           |
| *Community opinion* | Gardner expert vote              | 25%           |

![Gartner_point_system.png](/img/user/06%20Images/06.01%20Erazmus/Gartner_point_system.png)

The ranking is based on pints awarded for both “business metrics” and “community opinion”

ESG points are awarded for the following:
- **Commitment**
	- If the company follows UN Global Compact
- **Transparency**
	- If the company’s sustainability reports follow the Global Reporting Initiative standards
- **Performance**
	- Divided into 3 categories
		- Environmental
		- Social
		- Governance

> [!question]
> *What do you think of the evaluation criteria?*
> - Opinion on the distribution of points/percentages?
> - Is there a need to add more criteria?
> - What developments in the criteria do you expect to become relevant over the next 10 years?

# Qualitative [[03 Erazmus/03.11 Supply chain management/Benchmarking\|Benchmarking]]
- It is sometimes difficult to benchmark qualitatively
	- Paradox between **measurability and qualitative values**
	- Can provide a completely different perspective on one's Supply Chain Management when drawing inspiration from the *"best in class"*

> [!note]
> With qualitative benchmarking, it is natural to look outside one's own industry/sector (*in contrast to quantitative benchmarking*).